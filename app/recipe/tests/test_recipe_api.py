...
class PrivateRecipeApiTests(TestCase):
    ...
    def test_retrieve_recipes(self):
        ...
        recipes = Recipe.objects.all().order_by(
            '-id'
        )  # '-id' returns list in descending order of id
    ...
    def test_create_recipe(self):
        """Test creating a recipe using API."""
        payload = {
            'title': 'Sample recipe',
            'time_minutes': 30,
            'price': Decimal('5.99'),
    ...
    def test_partial_update(self):
        """Test partial update of the recipe (PATCH)."""
    ...
    def test_update_recipe_assign_tag(self):
        """Assign existing tag when updating a recipe."""
    ...
    def test_update_recipe_assign_ingredient(self):
        """Assign existing ingredient when updating a recipe."""
        ingredient1 = Ingredient.objects.create(user=self.user, name='Apple')
        recipe = create_recipe(user=self.user)
        recipe.ingredients.add(ingredient1)

        ingredient2 = Ingredient.objects.create(user=self.user, name='Grape')
        payload = {
            'ingredients': [{'name': 'Grape'}],
        }
        url = detail_url(recipe.id)
        res = self.client.patch(url, payload, format='json')
    ...
    def test_clear_recipe_ingredients(self):
        ...
        ingredient = Ingredient.objects.create(user=self.user, name='ingredient_to_be_removed')
        recipe = create_recipe(user=self.user)
...
class ImageUploadTests(TestCase):
    ...
    def test_upload_image(self):
        ...
        with tempfile.NamedTemporaryFile(suffix='.jpg') as image_file:
            #  using context manager to automatically clean up temp file
            img = Image.new('RGB', (10, 10))  # 10 x 10 pixel image
